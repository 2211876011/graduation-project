...
tb = SummaryWriter(os.path.join(CommonOpt['output_path'], 'dwsr/tensorboard'))

if __name__ == '__main__':
    model = DWSR(DWSROpt['n_conv'], DWSROpt['residue_weight'])
    model = model.cuda()

    # MSELoss, Adam optimizer
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=DWSROpt['lr'], weight_decay=1e-4, betas=(0.9, 0.999), eps=1e-8)
    # learning rate decays by x0.5 every 20 epochs
    scheduler = lr_scheduler.MultiStepLR(optimizer, list(range(20, DWSROpt['epochs'], 20)), gamma=0.5)
    ...
    # 训练
    for epoch in range(DWSROpt['epochs']):
        model.train()
        epoch_loss = AverageMeter()

        with tqdm(total=(len(train_set) - len(train_set) % DWSROpt['batch_size']), ncols=80, desc='Training') as t:
            t.set_description('Epoch: {}/{}'.format(epoch + 1, DWSROpt['epochs']))
            ...
            torch.save(model.state_dict(),
                       os.path.join(CommonOpt['output_path'], 'dwsr/ckp/', 'epoch_{}.pth'.format(epoch)))

            # 验证
            if epoch == 0 or (epoch + 1) % DWSROpt['val_every'] == 0:
                model.eval()
                ...
                for data in tqdm(val_loader, ncols=80, desc='Validation'):
                    ...
                    predict_rgb.save(
                        os.path.join(CommonOpt['output_path'], 'dwsr/val_inference/', '{}_rgb.png'.format(filename[0])))
                    # 保存单通道图
                    predict_y = Image.fromarray(predict_y, mode='L')
                    predict_y.save(
                        os.path.join(CommonOpt['output_path'], 'dwsr/val_inference/', '{}_y.png'.format(filename[0])))
                    # 保存输入的图片
                    noisy_y = Image.fromarray(np.array(lr.detach().cpu().numpy().squeeze().squeeze() * 255,
                                                       dtype=np.uint8),
                                              mode='L')
                    noisy_y.save(
                        os.path.join(CommonOpt['output_path'], 'dwsr/val_inference/', '{}_lr.png'.format(filename[0])))

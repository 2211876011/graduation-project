...
if __name__ == '__main__':
    ...
    scheduler = lr_scheduler.MultiStepLR(optimizer, list(range(100, EDSROpt['epochs'], 100)), gamma=0.5)
        ...
        with tqdm(total=(len(train_set) - len(train_set) % EDSROpt['batch_size']), ncols=80, desc='Training') as t:
            ...
            if epoch == 0 or (epoch + 1) % EDSROpt['val_every'] == 0:
                ...
                for data in tqdm(val_loader, ncols=80, desc='Validation'):
                    ...
                    predict_rgb.save(
                        os.path.join(CommonOpt['output_path'], 'edsr/val_inference/', '{}_rgb.png'.format(filename[0])))
                    # 保存单通道图
                    predict_y = Image.fromarray(predict_y, mode='L')
                    predict_y.save(
                        os.path.join(CommonOpt['output_path'], 'edsr/val_inference/', '{}_y.png'.format(filename[0])))
                    # 保存输入的图片

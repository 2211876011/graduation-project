...
BATCH_SIZE = 32
VAL_EVERY = 10
OUTPUT_PATH = r'/mnt/zhuoxu/code/graduation-project/nova/lu-mwcnn/output_deeplesion'
        ...
        with tqdm(total=(len(train_set) - len(train_set) % BATCH_SIZE), ncols=80, desc='Training') as t:
            ...
            if epoch == 0 or (epoch + 1) % VAL_EVERY == 0:
                ...
                for data in tqdm(val_loader, ncols=80, desc='Validation'):
                    ...
                    predict_gray.save(os.path.join(OUTPUT_PATH, './val_inference/', '{}_gray.png'.format(filename[0])))
                    # 保存输入的图片
